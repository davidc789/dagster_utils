...
class SqlTable(Config):
    ...
    def get_table(self, conn: Engine, metadata: MetaData | None = None):
        return self._reflect_table(conn, metadata=metadata)

    def get_table_name(self, conn: Engine, metadata: MetaData | None = None) -> str:
        ...
        """
        table = self._reflect_table(conn, metadata=metadata)
        # TODO: investigate the right value
        return table.name

    def drop(self, bind: Engine, checkfirst: bool = True, metadata: MetaData | None = None) -> None:
        table = self._reflect_table(bind, metadata)
        table.drop(bind=bind, checkfirst=checkfirst)

    def _reflect_table(self, conn: Engine, metadata: MetaData | None = None):
