...
from .sources import CsvSource, ExcelSource, ParquetSource, JsonSource
...
def output_status(context: dg.OpExecutionContext, backend: str, output: pd.DataFrame | duckdb.DuckDBPyTable):
    if backend == "pandas":
...
class ReadBuilder(object):
    read_handler: Callable[[Source], pd.DataFrame | duckdb.DuckDBPyTable]
    ...
    def build(self):
        if self.source.read_options.backend == "pandas":
            self.read_handler = lambda source: pd.read_csv(source.path, **source.read_options.model_dump(exclude_unset=True))
        else:
            self.read_handler = lambda source: duckdb.read_csv(source.path, **source.read_options.model_dump(exclude_unset=True))


def read_csv(source: CsvSource):
    def _wrapped_function(context: dg.OpExecutionContext, global_config: GlobalConfigResource):
        ...
        output_status(context, source.read_options.backend, output)
        return output

    return _wrapped_function


def read_excel(source: ExcelSource):
    def _wrapped_function(context: dg.OpExecutionContext, global_config: GlobalConfigResource):
...
def read_json(source: JsonSource):
    def _wrapped_function(context: dg.OpExecutionContext, global_config: GlobalConfigResource):
        if source.read_options.backend == "pandas":
...
def read_parquet(source: ParquetSource):
    def _wrapped_function(context: dg.OpExecutionContext, global_config: GlobalConfigResource):
        if source.read_options.backend == "pandas":
