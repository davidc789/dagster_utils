...
if TYPE_CHECKING:
    from typing import ClassVar, Any, Literal, Sequence, Hashable
    from pydantic import SkipJsonSchema, Annotated


class Source(dg.Model, dg.Resolvable):
...
class SourceComponent(dg.Component, dg.Model, dg.Resolvable):
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        asset = dg.observable_source_asset(
            name=source_object.name,
        )

        # Add definition construction logic here.
        return dg.Definitions(assets=[asset])


class DestinationWriteConfig(dg.Model, dg.Resolvable):
    pass


class SourceReadConfig(dg.Model, dg.Resolvable):
    pass
...
class ParquetReadOptions(dg.Model, dg.Resolvable):
    backend: Literal["pandas"] = Field(default="pandas", exclude=True)


class ParquetReadOptionsDuckdb(dg.Model, dg.Resolvable):
...
class JsonReadOptions(dg.Model, dg.Resolvable):
    backend: Literal["pandas"] = Field(default="pandas", exclude=True)


class JsonReadOptionsDuckdb(dg.Model, dg.Resolvable):
    backend: Literal["duckdb"] = Field(default="duckdb", exclude=True)
...
class SqlTableReadOptions(dg.Model, dg.Resolvable):
...
class SqlSource(dg.Model, dg.Resolvable):
...
class ElModel(dg.Model, dg.Resolvable):
    name: str
